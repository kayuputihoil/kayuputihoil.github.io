...
workbox.core.setCacheNameDetails({prefix:'workbox',suffix:'v6'})//jika ubah suffix jgn lupa ubah di suffix di event activate 
    ...
    caches.keys().then(keyList => {
      return Promise.all(
        keyList.filter(key => {
          ...
          return !key.startsWith('workbox') || !key.endsWith('v6');
        }).map(key => {
            console.log(key);
